...
def predict_MPG(data, user, pw, db):
    ...
    prediction = pd.DataFrame(model1.predict(clean_data1), columns = ['Predict_Performance'])
    
    final = pd.concat([prediction,data], axis = 1)
    final.to_sql('performance_predictions', con = engine, if_exists = 'replace', chunksize = 1000, index = False)
